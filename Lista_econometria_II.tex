...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{troca=}\KeywordTok{ifelse}\NormalTok{(V12_adj}\OperatorTok{>}\KeywordTok{lead}\NormalTok{(V12_adj),}\DecValTok{1}\NormalTok{,}\DecValTok{0}\NormalTok{), }
                ...
                \DataTypeTok{difV12=}\KeywordTok{ifelse}\NormalTok{(}\KeywordTok{is.na}\NormalTok{(difV12)}\OperatorTok{==}\NormalTok{T}\OperatorTok{|}\NormalTok{difV12}\OperatorTok{<}\DecValTok{0}\NormalTok{,V12_adj,difV12),}
                \DataTypeTok{diffV12_mult=}\KeywordTok{cut}\NormalTok{(difV12,}
                                 \KeywordTok{c}\NormalTok{(}\OperatorTok{-}\OtherTok{Inf}\NormalTok{,}\DecValTok{5000}\NormalTok{,}\DecValTok{1000}\NormalTok{,}\OtherTok{Inf}\NormalTok{),}
                                 \DataTypeTok{labels =}\NormalTok{ F))) }\CommentTok{# Ajustar primeiro valor}
...
\NormalTok{data=}\StringTok{ }\KeywordTok{rbind}\NormalTok{(g870[,}\KeywordTok{c}\NormalTok{(}\StringTok{"V1"}\NormalTok{,}\StringTok{"troca"}\NormalTok{,}\StringTok{"V12_adj"}\NormalTok{,}\StringTok{"estado"}\NormalTok{,}\StringTok{"difV12"}\NormalTok{,}\StringTok{"diffV12_mult"}\NormalTok{)],}
\NormalTok{            rt50[,}\KeywordTok{c}\NormalTok{(}\StringTok{"V1"}\NormalTok{,}\StringTok{"troca"}\NormalTok{,}\StringTok{"V12_adj"}\NormalTok{,}\StringTok{"estado"}\NormalTok{,}\StringTok{"difV12"}\NormalTok{,}\StringTok{"diffV12_mult"}\NormalTok{)],}
\NormalTok{            t8h203[,}\KeywordTok{c}\NormalTok{(}\StringTok{"V1"}\NormalTok{,}\StringTok{"troca"}\NormalTok{,}\StringTok{"V12_adj"}\NormalTok{,}\StringTok{"estado"}\NormalTok{,}\StringTok{"difV12"}\NormalTok{,}\StringTok{"diffV12_mult"}\NormalTok{)])}

  

\CommentTok{# Verificar a probabilidade empírica da mudança em cada estado}
\NormalTok{p <-}\StringTok{ }\NormalTok{data }\OperatorTok{%>%}
\StringTok{  }\CommentTok{# Criar as quebras de [0,5000],(5000,10000],(10000,Inf)}
...
\CommentTok{# l_1 <- glm(troca~difV12-1,family = "binomial",}
\CommentTok{#             data = data)}
\CommentTok{# coef(l_1)}
\CommentTok{# est = predict(l_1)}
\CommentTok{# prob0 = 1/(1+est)                           }
\CommentTok{# prob0 = cbind(1-prob0,prob0)}


\CommentTok{# l_2 <- nnet::multinom(diffV12_mult~lag(diffV12_mult)-1,family = "binomial",}
\CommentTok{#            data = data)}
\CommentTok{# est = l_2$fitted.values}
\CommentTok{# }
\CommentTok{# p <- c(mean(est[,1]),mean(est[,2]),mean(est[,3]))}

\NormalTok{rc=}\DecValTok{20}
\NormalTok{theta1_}\DecValTok{1}\NormalTok{=}\FloatTok{0.5}
\NormalTok{theta1_}\DecValTok{2}\NormalTok{=}\FloatTok{0.01}
\NormalTok{beta=}\FloatTok{0.98}
...
\begin{verbatim}
## Convergence achieved in  1060  iterations
\end{verbatim}
...
\StringTok{  }\KeywordTok{scale_x_continuous}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{seq}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{90}\NormalTok{,}\DataTypeTok{by=}\DecValTok{5}\NormalTok{))}\OperatorTok{+}
...
\CommentTok{# Aplicação à função custo}
...
\NormalTok{lin_fit =}\StringTok{ }\KeywordTok{optim}\NormalTok{(}\DataTypeTok{par=}\KeywordTok{rep}\NormalTok{(.}\DecValTok{1}\NormalTok{,npars),}\DataTypeTok{fn=}\NormalTok{dynamiclogit,}\DataTypeTok{method=}\KeywordTok{c}\NormalTok{(}\StringTok{"L-BFGS-B"}\NormalTok{),}
...
\begin{verbatim}
## Convergence achieved in  1092  iterations
\end{verbatim}
